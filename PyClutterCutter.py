...
def find_unused_files(directory):
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            if (not should_ignore(file_path, ignore_patterns, CONFIG['IGNORE_DIRS']) and
                any(file.endswith(ext) for ext in CONFIG['EXTENSIONS']) and
                (not CONFIG['EXCLUDE_SELF'] or file_path != current_script)):
